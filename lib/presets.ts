...
/** Presets bucketed by group, preserving first-seen group order. */
export function presetGroups(): { name: string; items: { preset: Preset; index: number }[] }[] {
  const order: string[] = [];
  const groups = new Map<string, { preset: Preset; index: number }[]>();

  PRESETS.forEach((preset, index) => {
    let bucket = groups.get(preset.group);
    if (!bucket) {
      bucket = [];
      groups.set(preset.group, bucket);
      order.push(preset.group);
    }
    bucket.push({ preset, index });
  });
